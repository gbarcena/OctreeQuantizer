...
oct_node nearest_color(int *v, node_heap *h)
{
    ...
    oct_node o = 0;
    for (i = 1; i < h->n; i++) {
        oct_node curBuff = h->buf[i];
        diff = 2 * abs((int)(curBuff->r - v[0]))
        + 4 * abs((int)(curBuff->g - v[1]))
        + abs((int)(curBuff->b - v[2]));
        if (diff < max) {
            max = diff;
            o = curBuff;
        }
...
#define POS(i, j) (4 * ((i) * im->w + (j)))
	int i, j;
	int *npx = calloc(sizeof(int), im->h * im->w * 4), *px;
	int v[4];
...
#define clamp(x, i) if (x[i] > 255) x[i] = 255; if (x[i] < 0) x[i] = 0
	...
	for (px = npx, i = 0; i < im->h; i++) {
		for (j = 0; j < im->w; j++, pix += 4, px += 4) {
			...
			px[2] /= CTOTAL;
            px[3] /= CTOTAL;
			clamp(px, 0); clamp(px, 1); clamp(px, 2); clamp(px, 3);
